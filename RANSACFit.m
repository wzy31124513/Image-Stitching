function H = RANSACFit(p1, p2, M, maxIter, seedSetSize, maxInlierError, goodFitThresh )
...
n=Inf;
for i=1:maxIter
    [D1,D2]=part(M,seedSetSize);
    temp=ComputeAffineMatrix(p1(D1(:,1),:),p2(D1(:,2),:));
    dists=ComputeError(temp,p1,p2,D2);
    count=0;
    index=[];
    for i=1:size(dists,1)
        if dists(i)<maxInlierError
            count=count+1;
            index=[index;i];
        end
    end
    if count+seedSetSize>=goodFitThresh
        M=[D1;D2(index,:)];
        temp=ComputeAffineMatrix(p1(M(:,1),:),p2(M(:,2),:));
        e=sum(ComputeError(temp,p1,p2,M));
        if e<n
            n=e;
            H=temp;
        end
...
function dists = ComputeError(H, pt1, pt2, M)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  M1=[p1' ; ones(1,size(M,1))];
  T=H*M1;
  T=T(1:2,:)';
  dist=T-p2;
  dists=sqrt(dist(:,1).^2+dist(:,2).^2);
...
%                                                                              %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if size(dists,1) ~= size(M,1) || size(dists,2) ~= 1
        error('wrong format');
    end
end
